...
exports.run = async (client, msg, params) => {
  ...
  if (msg.guild.member(user).roles.has(server.muterole)) {
    msg.channel.send(`<:wumpusxmark:599363387801075712> This user is not muted!`).catch((error) => {log.error(error)});
    return;
  }
  ...
  msg.guild.member(user).removeRole(server.muterole)
  const embed = new Discord.RichEmbed()
  .setAuthor(`${user.tag} has been unmuted`, user.avatarURL)
